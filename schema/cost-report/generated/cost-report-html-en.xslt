...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="c:Header">
    ...
    <div class="sub">Cost report as of <xsl:value-of select="c:Reporting/@asOf"/></div>
    <div class="meta">
      <div><span class="lbl">Production</span><span class="val"><xsl:value-of select="c:Production/c:Code"/></span></div>
      <div><span class="lbl">Report no.</span><span class="val"><xsl:value-of select="c:Reporting/@number"/></span></div>
      <div><span class="lbl">As of</span><span class="val"><xsl:value-of select="c:Reporting/@asOf"/></span></div>
      <div><span class="lbl">Against budget</span>
      ...
      <div><span class="lbl">Sender</span><span class="val"><xsl:value-of select="c:Transmission/c:Sender/c:Name"/></span></div>
      <xsl:if test="c:Transmission/c:Recipient">
        <div><span class="lbl">Recipient</span><span class="val"><xsl:value-of select="c:Transmission/c:Recipient/c:Name"/></span></div>
      </xsl:if>
      <xsl:if test="c:PreviousReport">
        <div><span class="lbl">Previous</span><span class="val">no. <xsl:value-of select="c:PreviousReport/@number"/></span></div>
      </xsl:if>
      <xsl:if test="c:Production/c:Currency">
        <div><span class="lbl">Currency</span><span class="val"><xsl:value-of select="c:Production/c:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template name="bodyTable">
    <table>
      <thead>
        <tr>
          <th class="l">Chapter / Account / Line</th>
          <th>Budget</th><th>Spent</th><th>Committed</th><th>Cost to date</th>
          <th>EFC</th><th>Variance</th><th>Est. to complete</th><th>Movement</th>
        </tr>
      </thead>
      <tbody>
