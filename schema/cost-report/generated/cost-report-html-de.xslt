...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/c:CostReport">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>Kostenbericht &#8211; <xsl:value-of select="c:Header/c:Production/c:Title"/></title>
        <style>
  ...
  <xsl:template match="c:Header">
    ...
    <div class="sub">Kostenbericht zum <xsl:value-of select="c:Reporting/@asOf"/></div>
    <div class="meta">
      <div><span class="lbl">Produktion</span><span class="val"><xsl:value-of select="c:Production/c:Code"/></span></div>
      <div><span class="lbl">Bericht Nr.</span><span class="val"><xsl:value-of select="c:Reporting/@number"/></span></div>
      <div><span class="lbl">Zum</span><span class="val"><xsl:value-of select="c:Reporting/@asOf"/></span></div>
      <div><span class="lbl">Gegen Budget</span>
      ...
      <div><span class="lbl">Absender</span><span class="val"><xsl:value-of select="c:Transmission/c:Sender/c:Name"/></span></div>
      <xsl:if test="c:Transmission/c:Recipient">
        <div><span class="lbl">Empfänger</span><span class="val"><xsl:value-of select="c:Transmission/c:Recipient/c:Name"/></span></div>
      </xsl:if>
      <xsl:if test="c:PreviousReport">
        <div><span class="lbl">Vorherig</span><span class="val">no. <xsl:value-of select="c:PreviousReport/@number"/></span></div>
      </xsl:if>
      <xsl:if test="c:Production/c:Currency">
        <div><span class="lbl">Währung</span><span class="val"><xsl:value-of select="c:Production/c:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template name="bodyTable">
    <table>
      <thead>
        <tr>
          <th class="l">Kapitel / Konto / Position</th>
          <th>Budget</th><th>Ausgegeben</th><th>Gebunden</th><th>Kosten bis dato</th>
          <th>Voraussichtl. Endkosten</th><th>Abweichung</th><th>Bis Fertigstellung</th><th>Veränderung</th>
        </tr>
      </thead>
      <tbody>
        <xsl:apply-templates select="c:Body/c:Chapter"/>
        <tr class="total">
          <td class="l">GESAMT</td>
