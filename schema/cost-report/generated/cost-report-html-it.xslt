...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/c:CostReport">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>Cost Report &#8211; <xsl:value-of select="c:Header/c:Production/c:Title"/></title>
        <style>
  ...
  <xsl:template match="c:Header">
    ...
    <div class="sub">Cost report al <xsl:value-of select="c:Reporting/@asOf"/></div>
    <div class="meta">
      <div><span class="lbl">Produzione</span><span class="val"><xsl:value-of select="c:Production/c:Code"/></span></div>
      <div><span class="lbl">Report n.</span><span class="val"><xsl:value-of select="c:Reporting/@number"/></span></div>
      <div><span class="lbl">Al</span><span class="val"><xsl:value-of select="c:Reporting/@asOf"/></span></div>
      <div><span class="lbl">Rispetto a budget</span>
      ...
      <div><span class="lbl">Mittente</span><span class="val"><xsl:value-of select="c:Transmission/c:Sender/c:Name"/></span></div>
      <xsl:if test="c:Transmission/c:Recipient">
        <div><span class="lbl">Destinatario</span><span class="val"><xsl:value-of select="c:Transmission/c:Recipient/c:Name"/></span></div>
      </xsl:if>
      <xsl:if test="c:PreviousReport">
        <div><span class="lbl">Precedente</span><span class="val">no. <xsl:value-of select="c:PreviousReport/@number"/></span></div>
      </xsl:if>
      <xsl:if test="c:Production/c:Currency">
        <div><span class="lbl">Valuta</span><span class="val"><xsl:value-of select="c:Production/c:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template name="bodyTable">
    <table>
      <thead>
        <tr>
          <th class="l">Capitolo / Conto / Voce</th>
          <th>Budget</th><th>Speso</th><th>Impegnato</th><th>Costo a oggi</th>
          <th>Costo a finire</th><th>Scostamento</th><th>Da completare</th><th>Movimento</th>
        </tr>
      </thead>
      <tbody>
