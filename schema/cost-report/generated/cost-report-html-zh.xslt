...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/c:CostReport">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>成本报告 &#8211; <xsl:value-of select="c:Header/c:Production/c:Title"/></title>
        <style>
  ...
  <xsl:template match="c:Header">
    ...
    <div class="sub">成本报告截至 <xsl:value-of select="c:Reporting/@asOf"/></div>
    <div class="meta">
      <div><span class="lbl">制作</span><span class="val"><xsl:value-of select="c:Production/c:Code"/></span></div>
      <div><span class="lbl">报告编号</span><span class="val"><xsl:value-of select="c:Reporting/@number"/></span></div>
      <div><span class="lbl">截至</span><span class="val"><xsl:value-of select="c:Reporting/@asOf"/></span></div>
      <div><span class="lbl">对照预算</span>
      ...
      <div><span class="lbl">发送方</span><span class="val"><xsl:value-of select="c:Transmission/c:Sender/c:Name"/></span></div>
      <xsl:if test="c:Transmission/c:Recipient">
        <div><span class="lbl">接收方</span><span class="val"><xsl:value-of select="c:Transmission/c:Recipient/c:Name"/></span></div>
      </xsl:if>
      <xsl:if test="c:PreviousReport">
        <div><span class="lbl">上一期</span><span class="val">no. <xsl:value-of select="c:PreviousReport/@number"/></span></div>
      </xsl:if>
      <xsl:if test="c:Production/c:Currency">
        <div><span class="lbl">货币</span><span class="val"><xsl:value-of select="c:Production/c:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template name="bodyTable">
    <table>
      <thead>
        <tr>
          <th class="l">章 / 科目 / 条目</th>
          <th>预算</th><th>已支出</th><th>已承诺</th><th>至今成本</th>
          <th>预计最终成本</th><th>差异</th><th>完工尚需</th><th>变动</th>
        </tr>
      </thead>
      <tbody>
