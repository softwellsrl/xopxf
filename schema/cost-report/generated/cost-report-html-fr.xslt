...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/c:CostReport">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>Rapport de coûts &#8211; <xsl:value-of select="c:Header/c:Production/c:Title"/></title>
        <style>
  ...
  <xsl:template match="c:Header">
    ...
    <div class="sub">Rapport de coûts au <xsl:value-of select="c:Reporting/@asOf"/></div>
    <div class="meta">
      <div><span class="lbl">Production</span><span class="val"><xsl:value-of select="c:Production/c:Code"/></span></div>
      <div><span class="lbl">Rapport n°</span><span class="val"><xsl:value-of select="c:Reporting/@number"/></span></div>
      <div><span class="lbl">Au</span><span class="val"><xsl:value-of select="c:Reporting/@asOf"/></span></div>
      ...
      <div><span class="lbl">Émetteur</span><span class="val"><xsl:value-of select="c:Transmission/c:Sender/c:Name"/></span></div>
      <xsl:if test="c:Transmission/c:Recipient">
        <div><span class="lbl">Destinataire</span><span class="val"><xsl:value-of select="c:Transmission/c:Recipient/c:Name"/></span></div>
      </xsl:if>
      <xsl:if test="c:PreviousReport">
        <div><span class="lbl">Précédent</span><span class="val">no. <xsl:value-of select="c:PreviousReport/@number"/></span></div>
      </xsl:if>
      <xsl:if test="c:Production/c:Currency">
        <div><span class="lbl">Devise</span><span class="val"><xsl:value-of select="c:Production/c:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template name="bodyTable">
    <table>
      <thead>
        <tr>
          <th class="l">Chapitre / Compte / Poste</th>
          <th>Budget</th><th>Dépensé</th><th>Engagé</th><th>Coût à ce jour</th>
          <th>Coût final estimé</th><th>Écart</th><th>Reste à engager</th><th>Mouvement</th>
        </tr>
      </thead>
      <tbody>
