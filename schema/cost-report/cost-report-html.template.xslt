...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/c:CostReport">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>@@doc_title_cr@@ &#8211; <xsl:value-of select="c:Header/c:Production/c:Title"/></title>
        <style>
  ...
  <xsl:template match="c:Header">
    ...
    <div class="sub">@@subtitle_cr@@ <xsl:value-of select="c:Reporting/@asOf"/></div>
    <div class="meta">
      <div><span class="lbl">@@lbl_production@@</span><span class="val"><xsl:value-of select="c:Production/c:Code"/></span></div>
      <div><span class="lbl">@@lbl_reportno@@</span><span class="val"><xsl:value-of select="c:Reporting/@number"/></span></div>
      <div><span class="lbl">@@lbl_asof@@</span><span class="val"><xsl:value-of select="c:Reporting/@asOf"/></span></div>
      <div><span class="lbl">@@lbl_againstbudget@@</span>
      ...
      <div><span class="lbl">@@lbl_sender@@</span><span class="val"><xsl:value-of select="c:Transmission/c:Sender/c:Name"/></span></div>
      <xsl:if test="c:Transmission/c:Recipient">
        <div><span class="lbl">@@lbl_recipient@@</span><span class="val"><xsl:value-of select="c:Transmission/c:Recipient/c:Name"/></span></div>
      </xsl:if>
      <xsl:if test="c:PreviousReport">
        <div><span class="lbl">@@lbl_previous@@</span><span class="val">no. <xsl:value-of select="c:PreviousReport/@number"/></span></div>
      </xsl:if>
      <xsl:if test="c:Production/c:Currency">
        <div><span class="lbl">@@lbl_currency@@</span><span class="val"><xsl:value-of select="c:Production/c:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template name="bodyTable">
    <table>
      <thead>
        <tr>
          <th class="l">@@col_tree_cr@@</th>
          <th>@@col_budget@@</th><th>@@col_spent@@</th><th>@@col_committed@@</th><th>@@col_costtodate@@</th>
          <th>@@col_efc@@</th><th>@@col_variance@@</th><th>@@col_esttocomplete@@</th><th>@@col_movement@@</th>
        </tr>
      </thead>
      <tbody>
