...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="b:Header">
    ...
    <div class="meta">
      <div>
        <span class="lbl">@@lbl_production@@</span>
        <span class="val"><xsl:value-of select="b:Production/b:Code"/></span>
      </div>
      <div>
        <span class="lbl">@@lbl_revision@@</span>
        <span class="val">
  ...
  <xsl:template name="fmt">
    ...
    <xsl:if test="$n != '' and $n != 0">
      <xsl:value-of select="format-number($n, '@@num_pattern@@', 'eu')"/>
    </xsl:if>
  </xsl:template>
