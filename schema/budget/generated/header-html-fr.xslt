...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="b:Header">
    ...
    <xsl:call-template name="codeName">
      <xsl:with-param name="title" select="'Départements'"/>
      <xsl:with-param name="rows"  select="b:Departments/b:Department"/>
    </xsl:call-template>
    <xsl:call-template name="producers"/>
    <xsl:call-template name="codeName">
      <xsl:with-param name="title" select="'Fonctions'"/>
      <xsl:with-param name="rows"  select="b:Roles/b:Role"/>
    </xsl:call-template>
    <xsl:call-template name="codeName">
      <xsl:with-param name="title" select="'Unités de mesure'"/>
    ...
    <xsl:call-template name="rates">
      <xsl:with-param name="title" select="'Taux de TVA'"/>
      <xsl:with-param name="rows"  select="b:VatRates/b:VatRate"/>
    </xsl:call-template>
    <xsl:call-template name="rates">
      <xsl:with-param name="title" select="'Taux de retenue'"/>
  ...
  <xsl:template name="groups">
    ...
    <xsl:choose>
      <xsl:when test="b:Groups/b:Group">
        <table>
          <thead><tr><th>Code</th><th>Nom</th><th>Schéma</th></tr></thead>
          <tbody>
      ...
      <xsl:otherwise><p class="empty">aucun déclaré</p></xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="phases">
    ...
    <xsl:choose>
      <xsl:when test="b:Phases/b:Phase">
        <table>
          <thead><tr><th>Code</th><th>Nom</th><th>Début</th><th>Fin</th></tr></thead>
          <tbody>
  ...
  <xsl:template name="producers">
    ...
    <xsl:choose>
      <xsl:when test="b:Producers/b:Producer">
        <table>
          <thead><tr><th>Code</th><th>Nom</th><th>N° TVA</th></tr></thead>
          <tbody>
  ...
  <xsl:template name="detailTypes">
    <h2>Types de détail</h2>
    <xsl:choose>
      <xsl:when test="b:DetailTypes/b:DetailType">
        <table>
          <thead><tr><th>Code</th><th>Nom</th><th>Réserve</th></tr></thead>
          <tbody>
            <xsl:for-each select="b:DetailTypes/b:DetailType">
              <tr>
                <td><span class="code"><xsl:value-of select="@code"/></span></td>
                <td><xsl:value-of select="@name"/></td>
                <td><xsl:if test="@contingency = 'true'">oui</xsl:if></td>
              </tr>
            </xsl:for-each>
          </tbody>
        </table>
  ...
  <xsl:template name="rates">
    ...
    <xsl:choose>
      <xsl:when test="$rows">
        <table>
          <thead><tr><th>Code</th><th>Nom</th><th class="num">Taux %</th></tr></thead>
          <tbody>
            <xsl:for-each select="$rows">
              <tr>
                <td><span class="code"><xsl:value-of select="@code"/></span></td>
