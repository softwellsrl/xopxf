...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="b:Header">
    ...
    <xsl:call-template name="rates">
      <xsl:with-param name="title" select="'增值税率'"/>
      <xsl:with-param name="rows"  select="b:VatRates/b:VatRate"/>
    </xsl:call-template>
    <xsl:call-template name="rates">
      <xsl:with-param name="title" select="'预扣税率'"/>
  ...
  <xsl:template name="groups">
    ...
    <xsl:choose>
      <xsl:when test="b:Groups/b:Group">
        <table>
          <thead><tr><th>代码</th><th>名称</th><th>方案</th></tr></thead>
          <tbody>
      ...
      <xsl:otherwise><p class="empty">未声明</p></xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="phases">
    ...
    <xsl:choose>
      <xsl:when test="b:Phases/b:Phase">
        <table>
          <thead><tr><th>代码</th><th>名称</th><th>开始</th><th>结束</th></tr></thead>
          <tbody>
  ...
  <xsl:template name="producers">
    ...
    <xsl:choose>
      <xsl:when test="b:Producers/b:Producer">
        <table>
          <thead><tr><th>代码</th><th>名称</th><th>增值税号</th></tr></thead>
          <tbody>
  ...
  <xsl:template name="detailTypes">
    ...
    <xsl:choose>
      <xsl:when test="b:DetailTypes/b:DetailType">
        <table>
          <thead><tr><th>代码</th><th>名称</th><th>储备金</th></tr></thead>
          <tbody>
            <xsl:for-each select="b:DetailTypes/b:DetailType">
              <tr>
                <td><span class="code"><xsl:value-of select="@code"/></span></td>
                <td><xsl:value-of select="@name"/></td>
                <td><xsl:if test="@contingency = 'true'">是</xsl:if></td>
              </tr>
            </xsl:for-each>
          </tbody>
        </table>
  ...
  <xsl:template name="rates">
    ...
    <xsl:choose>
      <xsl:when test="$rows">
        <table>
          <thead><tr><th>代码</th><th>名称</th><th class="num">税率 %</th></tr></thead>
          <tbody>
            <xsl:for-each select="$rows">
              <tr>
                <td><span class="code"><xsl:value-of select="@code"/></span></td>
