...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="b:Header">
    ...
    <xsl:call-template name="rates">
      <xsl:with-param name="title" select="'Mehrwertsteuersätze'"/>
      <xsl:with-param name="rows"  select="b:VatRates/b:VatRate"/>
    </xsl:call-template>
    <xsl:call-template name="rates">
      <xsl:with-param name="title" select="'Quellensteuersätze'"/>
  ...
  <xsl:template name="groups">
    ...
    <xsl:choose>
      <xsl:when test="b:Groups/b:Group">
        <table>
          <thead><tr><th>Code</th><th>Name</th><th>Schema</th></tr></thead>
          <tbody>
      ...
      <xsl:otherwise><p class="empty">keine angegeben</p></xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="phases">
    ...
    <xsl:choose>
      <xsl:when test="b:Phases/b:Phase">
        <table>
          <thead><tr><th>Code</th><th>Name</th><th>Beginn</th><th>Ende</th></tr></thead>
          <tbody>
  ...
  <xsl:template name="producers">
    ...
    <xsl:choose>
      <xsl:when test="b:Producers/b:Producer">
        <table>
          <thead><tr><th>Code</th><th>Name</th><th>USt-IdNr.</th></tr></thead>
          <tbody>
  ...
  <xsl:template name="detailTypes">
    ...
    <xsl:choose>
      <xsl:when test="b:DetailTypes/b:DetailType">
        <table>
          <thead><tr><th>Code</th><th>Name</th><th>Reserve</th></tr></thead>
          <tbody>
            <xsl:for-each select="b:DetailTypes/b:DetailType">
              <tr>
                <td><span class="code"><xsl:value-of select="@code"/></span></td>
                <td><xsl:value-of select="@name"/></td>
                <td><xsl:if test="@contingency = 'true'">ja</xsl:if></td>
              </tr>
            </xsl:for-each>
          </tbody>
        </table>
  ...
  <xsl:template name="rates">
    ...
    <xsl:choose>
      <xsl:when test="$rows">
        <table>
          <thead><tr><th>Code</th><th>Name</th><th class="num">Satz %</th></tr></thead>
          <tbody>
            <xsl:for-each select="$rows">
              <tr>
                <td><span class="code"><xsl:value-of select="@code"/></span></td>
