...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="b:Header">
    ...
    <xsl:call-template name="rates">
      <xsl:with-param name="title" select="'부가세율'"/>
      <xsl:with-param name="rows"  select="b:VatRates/b:VatRate"/>
    </xsl:call-template>
    <xsl:call-template name="rates">
      <xsl:with-param name="title" select="'원천징수율'"/>
  ...
  <xsl:template name="groups">
    ...
    <xsl:choose>
      <xsl:when test="b:Groups/b:Group">
        <table>
          <thead><tr><th>코드</th><th>이름</th><th>체계</th></tr></thead>
          <tbody>
      ...
      <xsl:otherwise><p class="empty">선언 없음</p></xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="phases">
    ...
    <xsl:choose>
      <xsl:when test="b:Phases/b:Phase">
        <table>
          <thead><tr><th>코드</th><th>이름</th><th>시작</th><th>종료</th></tr></thead>
          <tbody>
  ...
  <xsl:template name="producers">
    ...
    <xsl:choose>
      <xsl:when test="b:Producers/b:Producer">
        <table>
          <thead><tr><th>코드</th><th>이름</th><th>부가세 번호</th></tr></thead>
          <tbody>
  ...
  <xsl:template name="detailTypes">
    ...
    <xsl:choose>
      <xsl:when test="b:DetailTypes/b:DetailType">
        <table>
          <thead><tr><th>코드</th><th>이름</th><th>예비비</th></tr></thead>
          <tbody>
            <xsl:for-each select="b:DetailTypes/b:DetailType">
              <tr>
                <td><span class="code"><xsl:value-of select="@code"/></span></td>
                <td><xsl:value-of select="@name"/></td>
                <td><xsl:if test="@contingency = 'true'">예</xsl:if></td>
              </tr>
            </xsl:for-each>
          </tbody>
        </table>
  ...
  <xsl:template name="codeName">
    ...
    <xsl:choose>
      <xsl:when test="$rows">
        <table>
          <thead><tr><th>코드</th><th>이름</th></tr></thead>
          <tbody>
  ...
  <xsl:template name="rates">
    ...
    <xsl:choose>
      <xsl:when test="$rows">
        <table>
          <thead><tr><th>코드</th><th>이름</th><th class="num">세율 %</th></tr></thead>
          <tbody>
            <xsl:for-each select="$rows">
              <tr>
                <td><span class="code"><xsl:value-of select="@code"/></span></td>
