...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/d:Details">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>Details &#8211; <xsl:value-of select="d:Header/d:Production/d:Title"/></title>
        <style>
      ...
      <body>
        ...
        <xsl:if test="not(d:Header/d:Parties)">
          <p class="note">Counterparty registry omitted: only payee codes are shown (data minimisation).</p>
        </xsl:if>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template match="d:Header">
    ...
    <div class="sub">Analytic movements behind the cost report</div>
    <div class="meta">
      ...
      <div><span class="lbl">Document ID</span><span class="val"><xsl:value-of select="d:Transmission/d:DocumentId"/></span></div>
      <div><span class="lbl">Sender</span><span class="val"><xsl:value-of select="d:Transmission/d:Sender/d:Name"/></span></div>
      <xsl:if test="d:Transmission/d:Recipient">
        <div><span class="lbl">Recipient</span><span class="val"><xsl:value-of select="d:Transmission/d:Recipient/d:Name"/></span></div>
      </xsl:if>
      <xsl:if test="d:Production/d:Currency">
        <div><span class="lbl">Currency</span><span class="val"><xsl:value-of select="d:Production/d:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template name="bodyTable">
    <table>
      <thead>
        <tr>
          <th class="l">Chapter / Account / Movement</th>
          <th>Date</th>
          <th class="l">Counterparty</th>
