...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/d:Details">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>明细 &#8211; <xsl:value-of select="d:Header/d:Production/d:Title"/></title>
        <style>
      ...
      <body>
        ...
        <xsl:if test="not(d:Header/d:Parties)">
          <p class="note">已省略对方名录：仅显示代码（数据最小化）。</p>
        </xsl:if>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template match="d:Header">
    ...
    <div class="sub">成本报告背后的分析性流水</div>
    <div class="meta">
      <div><span class="lbl">制作</span><span class="val"><xsl:value-of select="d:Production/d:Code"/></span></div>
      <div><span class="lbl">对照预算</span>
           <span class="val">rev <xsl:value-of select="d:BudgetReference/@revision"/></span></div>
      <div><span class="lbl">成本报告</span>
      ...
      <div><span class="lbl">文件编号</span><span class="val"><xsl:value-of select="d:Transmission/d:DocumentId"/></span></div>
      <div><span class="lbl">发送方</span><span class="val"><xsl:value-of select="d:Transmission/d:Sender/d:Name"/></span></div>
      <xsl:if test="d:Transmission/d:Recipient">
        <div><span class="lbl">接收方</span><span class="val"><xsl:value-of select="d:Transmission/d:Recipient/d:Name"/></span></div>
      </xsl:if>
      <xsl:if test="d:Production/d:Currency">
        <div><span class="lbl">货币</span><span class="val"><xsl:value-of select="d:Production/d:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
    ...
    <tr class="movement">
      <td class="l">
        ...
        <xsl:if test="d:Document/@url">
          <a class="doc" href="{d:Document/@url}">文档</a>
        </xsl:if>
      </td>
      ...
      <td class="l">
        ...
        <xsl:if test="key('kType',@type)/@payroll = 'true'">
          <span class="tag payroll">工资</span>
        </xsl:if>
