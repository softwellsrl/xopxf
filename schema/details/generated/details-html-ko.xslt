...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/d:Details">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>세부내역 &#8211; <xsl:value-of select="d:Header/d:Production/d:Title"/></title>
        <style>
      ...
      <body>
        ...
        <xsl:if test="not(d:Header/d:Parties)">
          <p class="note">거래상대 명부 생략: 코드만 표시됩니다(데이터 최소화).</p>
        </xsl:if>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template match="d:Header">
    ...
    <div class="sub">비용 보고서의 근거가 되는 분석적 거래</div>
    <div class="meta">
      <div><span class="lbl">제작</span><span class="val"><xsl:value-of select="d:Production/d:Code"/></span></div>
      <div><span class="lbl">예산 대비</span>
           <span class="val">rev <xsl:value-of select="d:BudgetReference/@revision"/></span></div>
      <div><span class="lbl">비용 보고서</span>
      ...
      <div><span class="lbl">문서 ID</span><span class="val"><xsl:value-of select="d:Transmission/d:DocumentId"/></span></div>
      <div><span class="lbl">발신자</span><span class="val"><xsl:value-of select="d:Transmission/d:Sender/d:Name"/></span></div>
      <xsl:if test="d:Transmission/d:Recipient">
        <div><span class="lbl">수신자</span><span class="val"><xsl:value-of select="d:Transmission/d:Recipient/d:Name"/></span></div>
      </xsl:if>
      <xsl:if test="d:Production/d:Currency">
        <div><span class="lbl">통화</span><span class="val"><xsl:value-of select="d:Production/d:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
    ...
    <tr class="movement">
      <td class="l">
        ...
        <xsl:if test="d:Document/@url">
          <a class="doc" href="{d:Document/@url}">문서</a>
        </xsl:if>
      </td>
      ...
      <td class="l">
        ...
        <xsl:if test="key('kType',@type)/@payroll = 'true'">
          <span class="tag payroll">급여</span>
        </xsl:if>
      </td>
