...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/d:Details">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>Dettagli &#8211; <xsl:value-of select="d:Header/d:Production/d:Title"/></title>
        <style>
      ...
      <body>
        ...
        <xsl:if test="not(d:Header/d:Parties)">
          <p class="note">Anagrafica controparti omessa: vengono mostrati solo i codici (minimizzazione dei dati).</p>
        </xsl:if>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template match="d:Header">
    ...
    <div class="sub">Movimenti analitici a supporto del cost report</div>
    <div class="meta">
      <div><span class="lbl">Produzione</span><span class="val"><xsl:value-of select="d:Production/d:Code"/></span></div>
      <div><span class="lbl">Rispetto a budget</span>
           <span class="val">rev <xsl:value-of select="d:BudgetReference/@revision"/></span></div>
      <div><span class="lbl">Cost report</span>
      ...
      <div><span class="lbl">ID documento</span><span class="val"><xsl:value-of select="d:Transmission/d:DocumentId"/></span></div>
      <div><span class="lbl">Mittente</span><span class="val"><xsl:value-of select="d:Transmission/d:Sender/d:Name"/></span></div>
      <xsl:if test="d:Transmission/d:Recipient">
        <div><span class="lbl">Destinatario</span><span class="val"><xsl:value-of select="d:Transmission/d:Recipient/d:Name"/></span></div>
      </xsl:if>
      <xsl:if test="d:Production/d:Currency">
        <div><span class="lbl">Valuta</span><span class="val"><xsl:value-of select="d:Production/d:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template name="bodyTable">
    <table>
      <thead>
        <tr>
          <th class="l">Capitolo / Conto / Movimento</th>
          <th>Data</th>
          <th class="l">Controparte</th>
          <th class="l">Descrizione</th>
          <th>Speso</th>
          <th>Ordinato</th>
        </tr>
      </thead>
    ...
    <tr class="movement">
      <td class="l">
        ...
        <xsl:if test="d:Document/@url">
          <a class="doc" href="{d:Document/@url}">documento</a>
        </xsl:if>
      </td>
      ...
      <td class="l">
        ...
        <xsl:if test="key('kType',@type)/@payroll = 'true'">
          <span class="tag payroll">payroll</span>
        </xsl:if>
