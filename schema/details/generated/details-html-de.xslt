...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/d:Details">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>Einzelnachweise &#8211; <xsl:value-of select="d:Header/d:Production/d:Title"/></title>
        <style>
      ...
      <body>
        ...
        <xsl:if test="not(d:Header/d:Parties)">
          <p class="note">Gegenpartei-Verzeichnis weggelassen: nur Codes werden angezeigt (Datenminimierung).</p>
        </xsl:if>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template match="d:Header">
    ...
    <div class="sub">Analytische Bewegungen zum Kostenbericht</div>
    <div class="meta">
      <div><span class="lbl">Produktion</span><span class="val"><xsl:value-of select="d:Production/d:Code"/></span></div>
      <div><span class="lbl">Gegen Budget</span>
           <span class="val">rev <xsl:value-of select="d:BudgetReference/@revision"/></span></div>
      <div><span class="lbl">Kostenbericht</span>
      ...
      <div><span class="lbl">Dokument-ID</span><span class="val"><xsl:value-of select="d:Transmission/d:DocumentId"/></span></div>
      <div><span class="lbl">Absender</span><span class="val"><xsl:value-of select="d:Transmission/d:Sender/d:Name"/></span></div>
      <xsl:if test="d:Transmission/d:Recipient">
        <div><span class="lbl">Empfänger</span><span class="val"><xsl:value-of select="d:Transmission/d:Recipient/d:Name"/></span></div>
      </xsl:if>
      <xsl:if test="d:Production/d:Currency">
        <div><span class="lbl">Währung</span><span class="val"><xsl:value-of select="d:Production/d:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template name="bodyTable">
    <table>
      <thead>
        <tr>
          <th class="l">Kapitel / Konto / Bewegung</th>
          <th>Datum</th>
          <th class="l">Gegenpartei</th>
          <th class="l">Beschreibung</th>
          <th>Ausgegeben</th>
          <th>Bestellt</th>
        </tr>
      </thead>
    ...
    <tr class="movement">
      <td class="l">
        ...
        <xsl:if test="d:Document/@url">
          <a class="doc" href="{d:Document/@url}">Dokument</a>
        </xsl:if>
      </td>
      ...
      <td class="l">
        ...
        <xsl:if test="key('kType',@type)/@payroll = 'true'">
          <span class="tag payroll">Lohn</span>
        </xsl:if>
