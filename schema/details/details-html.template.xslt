...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/d:Details">
    <html lang="{@language}">
      <head>
        <meta charset="UTF-8"/>
        <title>@@doc_title_det@@ &#8211; <xsl:value-of select="d:Header/d:Production/d:Title"/></title>
        <style>
      ...
      <body>
        ...
        <xsl:if test="not(d:Header/d:Parties)">
          <p class="note">@@det_minimized_note@@</p>
        </xsl:if>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template match="d:Header">
    ...
    <div class="sub">@@subtitle_det@@</div>
    <div class="meta">
      <div><span class="lbl">@@lbl_production@@</span><span class="val"><xsl:value-of select="d:Production/d:Code"/></span></div>
      <div><span class="lbl">@@lbl_againstbudget@@</span>
           <span class="val">rev <xsl:value-of select="d:BudgetReference/@revision"/></span></div>
      <div><span class="lbl">@@det_lbl_costreport@@</span>
      ...
      <div><span class="lbl">@@lbl_docid@@</span><span class="val"><xsl:value-of select="d:Transmission/d:DocumentId"/></span></div>
      <div><span class="lbl">@@lbl_sender@@</span><span class="val"><xsl:value-of select="d:Transmission/d:Sender/d:Name"/></span></div>
      <xsl:if test="d:Transmission/d:Recipient">
        <div><span class="lbl">@@lbl_recipient@@</span><span class="val"><xsl:value-of select="d:Transmission/d:Recipient/d:Name"/></span></div>
      </xsl:if>
      <xsl:if test="d:Production/d:Currency">
        <div><span class="lbl">@@lbl_currency@@</span><span class="val"><xsl:value-of select="d:Production/d:Currency"/></span></div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template name="bodyTable">
    <table>
      <thead>
        <tr>
          <th class="l">@@det_col_tree@@</th>
          <th>@@det_col_date@@</th>
          <th class="l">@@det_col_payee@@</th>
          <th class="l">@@det_col_desc@@</th>
          <th>@@det_col_spent@@</th>
          <th>@@det_col_ordered@@</th>
        </tr>
      </thead>
      <tbody>
    ...
    <tr class="movement">
      <td class="l">
        ...
        <xsl:if test="d:Document/@url">
          <a class="doc" href="{d:Document/@url}">@@det_document@@</a>
        </xsl:if>
      </td>
      ...
      <td class="l">
        ...
        <xsl:if test="key('kType',@type)/@payroll = 'true'">
          <span class="tag payroll">@@det_payroll@@</span>
        </xsl:if>
      </td>
